#ifndef BEST_FIT_DIV_T_
#define BEST_FIT_DIV_T_
...
#include "fit_behav_tanh.hpp"

namespace sferes {
  namespace stat {
    SFERES_STAT(BestFitDivT, Stat) {
    public:
      template<typename E>
      void refresh(const E& ea) {
        ...
        if (_cnt == Params::pop::nb_gen){
          ...
          //std::string filename_out = "/git/sferes2/results_tanhnb_nov/novelty_gte.txt";
    ...
    Eigen::MatrixXd run_simu(T & model, Eigen::Vector3d target) { 
        ...
        for (int t=0; t< _t_max/_delta_t; ++t){
              ...
              for (int indx = 0; indx < 3; ++indx){
                ...
                output[indx] = (model.nn().get_outf(indx))*_vmax;
