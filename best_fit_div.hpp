#ifndef BEST_FIT_DIV_
#define BEST_FIT_DIV_
  ...
  namespace stat {
    ...
    public:
      ...
      void refresh(const E& ea) {
        ...
        if (_cnt == Params::pop::nb_gen){
          ...
          std::string filename_out = ea.res_dir()+ "/novelty_gte.txt";
    ...
    Eigen::MatrixXd run_simu(T & model, Eigen::Vector3d target) { 
        ...
        for (int t=0; t< _t_max/_delta_t; ++t){
              ...
              for (int j = 0; j < 20; ++j) //In case of non-FF NN
                model.nn().step(inputs);
